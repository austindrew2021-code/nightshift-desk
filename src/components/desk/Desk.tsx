...
export function Desk() {
  ...
  const modeHint = useMemo(() => {
    switch (engine.mode) {
      ...
      case "ict":
        return `ICT ${engine.ictFilter} · ${engine.ictStyle === "cisd" ? "CISD 5m A+" : engine.ictStyle ?? "all"} · ${engine.ictStyle === "cisd" ? "5m" : engine.ictUse5m === false ? "15m" : "15m+5m"} · $${startUsd.toFixed(0)} · ${engine.ictLev || 40}x iso liq ~${((1 / Math.max(2, engine.ictLev || 40) - 0.005) * 100).toFixed(1)}% · 1R ${engine.ictRegime === "expand" ? "22% expand" : `${(((engine.ictRiskPct || 0.18) * 100)).toFixed(0)}%`} · ¾@0.75R trail 5R · ${engine.ictRegime ?? "chop"} · ${engine.ictStyle === "cisd" ? "no Silver · no 15m · no Playback" : engine.ictRegimeNote || "A+ CISD still live"}.`;
      case "zostaff":
        ...
        <section className="flex flex-col justify-between bg-surface p-4 md:col-span-3">
          <div>
            ...
            <p className="mt-1 font-mono text-[11px] text-subtle tabular">
              {engine.ictStyle === "cisd" ? "5m CISD A+" : engine.ictUse5m === false ? "15m" : "15m+5m"} · {engine.ictLev || 40}x iso liq {((1 / Math.max(2, engine.ictLev || 40) - 0.005) * 100).toFixed(1)}% · 50% margin · 1R {engine.ictRegime === "expand" ? "22% expand" : `${((engine.ictRiskPct || 0.18) * 100).toFixed(0)}%`} · ¾@0.75R trail 5R · bank 25% / +$200 · lock 50% when expand dies
            </p>
            <p className="mt-1 font-mono text-[11px] text-subtle tabular">
