...
export function Desk() {
  ...
  const modeHint = useMemo(() => {
    switch (engine.mode) {
      ...
      case "ict":
        return `ICT ${engine.ictFilter} · ${engine.ictStyle ?? "all"} · ${engine.ictUse5m === false ? "15m" : "15m+5m"} · $${startUsd.toFixed(0)} · ${engine.ictLev || 40}x iso liq ~${((1 / Math.max(2, engine.ictLev || 40) - 0.005) * 100).toFixed(1)}% · 1R ${(((engine.ictRiskPct || 0.18) * 100)).toFixed(0)}% · take 1R full · ${engine.ictRegime ?? "chop"} · ${engine.ictRegimeNote || "A+ CISD still live"}.`;
      case "zostaff":
