...
const ICT = [
  ...
  { t: "A+ / range / 20x×50%", d: "A trade needs a liquidity raid, CISD, FVG or Unicorn in premium/discount (OTE 62–79 of the impulse when it overlaps), and ≥1.7R to the next draw on liquidity. Size default: 20×, 50% margin, 1R = 18% (chips: 10/15/20× and 12/18/30% 1R). Half off at 1R, then the stop ratchets BE → +0.5R → +1R → +1.5R → +2R → +3R as price runs, runner to 5R. Bank 25% of each +$200. Daily halt ≈ one full 1R at 18% (32% at 30% 1R). Last 15 days, session models + ratchet + 20×/18%: 24 fills, 88% win, $100 → $250. Same tape at 20×/30% was $248 — extra size does not pay after the halt. Full-size (no 50% partial) collapsed to 32% win. 30% 1R is a chip if you want it; two losers in a row is −60%." },
  ...
  { t: "Flash crash / liquidity grab", d: "Desks run stops on purpose: an ATR-spike wick through PDH, PDL, Asia, equal highs/lows or the last swing, then a close back inside. That is the grab. We long the reclaim of a low (flash crash), short the reclaim of a high. Same windows they hunt — London, Judas, 9:30 cash, Silver Bullet, funding 00/08/16 UTC. A wick alone is not a trade; it still needs CISD or an FVG the other way. GRAB on the chart marks those wicks so you can see them even when we pass." },
